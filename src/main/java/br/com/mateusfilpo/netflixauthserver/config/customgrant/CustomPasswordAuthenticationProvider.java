package br.com.mateusfilpo.netflixauthserver.config.customgrant;
...
import br.com.mateusfilpo.netflixauthserver.dto.RoleDTO;
...
public class CustomPasswordAuthenticationProvider implements AuthenticationProvider {
	...
	public CustomPasswordAuthenticationProvider(OAuth2AuthorizationService authorizationService,
			...
			AuthClient authClient, PasswordEncoder passwordEncoder) {
			// UserDetailsService userDetailsService, PasswordEncoder passwordEncoder) {
		
		Assert.notNull(authorizationService, "authorizationService cannot be null");
		Assert.notNull(tokenGenerator, "TokenGenerator cannot be null");
		Assert.notNull(authClient, "AuthClient cannot be null");
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		try {
			user = authClient.authenticateUser(new AuthRequestDTO(username, password)).block();
		...
		authorizedScopes = user.getRoles().stream()
				.map(scope -> scope.getRole())
		...
		for (RoleDTO dto : user.getRoles()) {
			Role role = new Role();
			role.setAuthority(dto.getRole());
			rolesSet.add(role);
		}
		CustomUserAuthorities customPasswordUser = new CustomUserAuthorities(username, rolesSet);
